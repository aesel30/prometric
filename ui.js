(function () {
  ...
  const renderDropdown = () => {
    dropdown.innerHTML = '<option value="">-- Pilih Akun Aktif --</option>' + accounts.map((acc, i) => `<option value="${i}" ${activeIdx == i ? "selected" : ""}>${acc.nama}</option>`).join("");
  };

  const updateForm = () => {
    const inNama = document.getElementById("in-nama"),
      inId = document.getElementById("in-id"),
      inPass = document.getElementById("in-pass"),
  ...
  document.getElementById("btn-save").onclick = () => {
    const n = document.getElementById("in-nama").value,
      i = document.getElementById("in-id").value,
  ...
  document.getElementById("btn-exp").onclick = () => {
    if (accounts.length === 0) return alert("Tidak ada data untuk di-export!");

    let dataToExport, fileName;

    // Jika ada akun yang sedang aktif/dipilih
    if (activeIdx !== null && accounts[activeIdx]) {
      const acc = accounts[activeIdx];
      dataToExport = acc; // Export hanya 1 akun
      fileName = `${acc.nama.replace(/\s+/g, "_")}.json`; // Contoh: Aesel.json
    } else {
      // Jika tidak pilih akun, export semua sebagai backup
      dataToExport = accounts;
      fileName = `Backup_Semua_Akun.json`;
    }
    ...
    console.log(`✅ [Export] Berhasil mengunduh: ${fileName}`);
  };
